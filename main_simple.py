#!/usr/bin/env python3
"""
Telegram бот для создания скриншотов веб-сайтов (версия без APScheduler)
...
class ScreenshotBot:
    def __init__(self):
        ...
        self.scheduler_tasks = {}  # Хранение задач планировщика
    ...
    def start_scheduler_for_user(self, user_id: int, application):
        """Запустить планировщик для пользователя"""
        settings = user_settings[user_id]
        
        # Создаем задачу планировщика
        task = asyncio.create_task(
            self._scheduler_loop(user_id, application)
        )
        self.scheduler_tasks[user_id] = task
        logger.info(f"Планировщик запущен для пользователя {user_id}")
    
    def stop_scheduler_for_user(self, user_id: int):
        """Остановить планировщик для пользователя"""
        if user_id in self.scheduler_tasks:
            self.scheduler_tasks[user_id].cancel()
            del self.scheduler_tasks[user_id]
            logger.info(f"Планировщик остановлен для пользователя {user_id}")
    
    async def _scheduler_loop(self, user_id: int, application):
        """Цикл планировщика для пользователя"""
        while True:
            try:
                ...
                now = datetime.utcnow()
                target_time = datetime.strptime(settings['schedule_time'], '%H:%M').time()
                ...
                logger.info(f"Пользователь {user_id}: следующий скриншот через {sleep_seconds/3600:.1f} часов")
                
                await asyncio.sleep(sleep_seconds)
                
                # Отправляем скриншот
                if settings['schedule_enabled']:  # Проверяем еще раз перед отправкой
                    await self.send_scheduled_screenshot(user_id, application)
                
            except asyncio.CancelledError:
                logger.info(f"Планировщик отменен для пользователя {user_id}")
                break
            except Exception as e:
                logger.error(f"Ошибка в планировщике для пользователя {user_id}: {e}")
    ...
    def run(self):
        ...
        # Запускаем бота
        logger.info("Бот запущен (версия без APScheduler)")
